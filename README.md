...
- [ ] Explain what a token is used for.
      A token is used for Authorization. It is sent to a server and then the server acknowledges that the token exists and allows for a user to gain access to the server.

- [ ] What steps can you take in your web apps to keep your data secure?
      Using server Authorization is one measure in making sure that the data is secured. Another way is to utilize PrivateRoutes which pushes users to login if they route wasn't authorized via a token.
- [ ] Describe how web servers work.
      Web servers stores code for websites. It can either store the code for a website or can be the program that runs on the server. A user makes a request to a dns, a request is made to the server and the server returns some data.
- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
      both .get's, both .post's, .delete, .put
